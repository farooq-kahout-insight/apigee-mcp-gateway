...
process, so the process holds something that can spend money, and the audit log
has a hole in it exactly where the reasoning happened. These agents close that
gap. There is no provider credential anywhere in the process.
...
`agent` and the same `client_key_fp`. So "what did agent-reader do at 14:03" has
an answer that covers the thinking and not just the doing.
...
reader read-only.** Its instruction says so, and an instruction is only a
suggestion. What actually stops it is that `AGENT_READER_KEY` belongs to the
...
audit record naming `agent-reader`. That is the demo, not a bug.
...
an agent module is first imported, which reads as if the whole agent stack were
missing. So the smoke suite skips instead of failing, and the real fix is one
...
`AGENT_API_KEY` exists because the two ways of running an agent want different
spellings. `adk run adk-agents/airlock_reader` is one agent in one process, and
the generic name reads naturally there. `adk web` serves both agents from a
single process, where one generic name cannot mean two different keys. The
specific name wins when both are set, so the second case cannot quietly give
both agents whichever key was exported last.

### What it refuses to start with

A scheme or a path in `APIGEE_HOST` is refused. Accepting one would let a config
change redirect every model call and every tool call somewhere else — the attack
this whole architecture exists to prevent, arriving through a config file
instead of through a prompt.
...
That is the same contract `mcp-server/server.py` enforces, plus the model-plane
names. `OPENAI_API_KEY` is the sharpest of them: **LiteLLM reads it from the
...
Importing `airlock_common` sets `LITELLM_MODE=PRODUCTION`, and that line does
real work. LiteLLM's `__init__` runs `dotenv.load_dotenv()` unless
`LITELLM_MODE` holds something other than `DEV`, and `load_dotenv` walks up from
site-packages until it finds a `.env`. For an agent run out of this repository,
that is the `.env` at the root: the one `provision.sh` reads to fill the KVM,
holding `OPENROUTER_API_KEY` and the GitHub PAT.

So `import litellm` puts both into the process by itself, whatever the launcher
unset. That is worse than the case the startup check was written for. There,
...
stripping it. If that import lands after `settings()` has run, the agent holds a
provider key, every check has passed, and nothing anywhere says so.

The value in the environment is overwritten on purpose rather than respected.
...
language, because its instruction tells it to report a 403 rather than look for
another route. There is no other route, and looking for one is the behaviour the
gateway is watching for.
...
Its exit codes are the interesting part, because a failing test run in this repo
should mean the gateway did something wrong, and nothing else:
...
so the refusals above can be tested on a machine that could not run an agent at
